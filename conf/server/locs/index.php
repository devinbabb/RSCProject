...
if($_GET['dir']) {
	...
	if($_GET['dir'] == "quests/") {
		header("Location: http://rscangel.org/dev/files/pub/RSCA/src/rscproject/gs/plugins/quests");
	}
	$dirok = true;
...
<link rel="stylesheet" type="text/css" href="http://rscangel.org/dev/files/pub/dlf/styles.css" />
<?
if($showthumbnails) {
?>
...
<!--
function o(n, i) {
	document.images['thumb'+n].src = 'http://rscangel.org/dev/files/pub/dlf/i.php?f='+i;

}

function f(n) {
	document.images['thumb'+n].src = 'http://rscangel.org/dev/files/pub/dlf/trans.gif';
}
//-->
</script>
<?
...
<div id="container">
  ...
  <div id="listingcontainer">
	...
	if($dirok) {
	?>
	<div><a href="<?=$_SERVER['PHP_SELF'].'?dir='.urlencode($dotdotdir);?>" class="<?=$class;?>"><img src="http://rscangel.org/dev/files/pub/dlf/dirup.png" alt="Folder" /><strong>..</strong> <em>-</em> <?=date ("M d Y h:i:s A", filemtime($dotdotdir));?></a></div>
	<?
		if($class=='b') $class='w';
		else $class = 'b';
	}
	...
	for($i=0;$i<$arsize;$i++) {
	?>
	<div><a href="<?=$_SERVER['PHP_SELF'].'?dir='.urlencode($leadon.$dirs[$i]);?>" class="<?=$class;?>"><img src="http://rscangel.org/dev/files/pub/dlf/folder.png" alt="<?=$dirs[$i];?>" /><strong><?=$dirs[$i];?></strong> <em>-</em> <?=date ("M d Y h:i:s A", filemtime($leadon.$dirs[$i]));?></a></div>
	<?
		if($class=='b') $class='w';
		else $class = 'b';	
	}
	...
	for($i=0;$i<$arsize;$i++) {
		...
		if($showthumbnails && in_array($ext, $supportedimages)) {
			$thumb = '<span><img src="http://rscangel.org/dev/files/pub/dlf/trans.gif" alt="'.$files[$i].'" name="thumb'.$i.'" /></span>';
	...
	?>
	<div><a href="<?=$fileurl;?>" class="<?=$class;?>"<?=$thumb2;?>><img src="http://rscangel.org/dev/files/pub/dlf/<?=$icon;?>" alt="<?=$files[$i];?>" /><strong><?=$filename;?></strong> <em><?=round(filesize($leadon.$files[$i])/1024);?>KB</em> <?=date ("M d Y h:i:s A", filemtime($leadon.$files[$i]));?><?=$thumb;?></a></div>
	<?
		if($class=='b') $class='w';
		else $class = 'b';	
	}	
