...
public class IRC extends PircBot implements Runnable {
	...
	public void handleReport(String sender, String target, int id) {
		sendMessage(IRCTools.bold() + IRCTools.getColor("red") + sender
				+ IRCTools.bold() + IRCTools.getColor("lightgreen")
				+ " has reported " + IRCTools.bold() + IRCTools.getColor("red")
				+ target + IRCTools.bold() + IRCTools.getColor("lightgreen")
				+ " for " + IRCTools.bold() + IRCTools.getColor("blue")
				+ IRCTools.getReportName(id));
	}
	...
	public void notifyAdmin(String user, String message) {
		if (Constants.IRC.USE_IRC) {
			...
			for (String admin : Constants.IRC.ADMINISTRATORS) {
				sendMessage(
						admin,
						IRCTools.bold() + IRCTools.getColor("blue") + user
								+ IRCTools.bold()
								+ IRCTools.getColor("lightgreen") + " "
								+ message);
			}
		}
	}
	...
	public static class IRCTools {
		...
		public static String getColor(String s) {
			...
			String[] colors = { "red", "yellow", "lightgreen", "blue",
					"purple", "white", "darkgreen", "orange" };
		...
		public static String getReportName(int id) {
			id--;
			String[] reports = { "Offensive Language", "Item Scamming",
					"Password Scamming", "Bug Abuse", "Staff Impersonation",
					"Account sharing/trading", "Autoing/Macroing",
		...
		public static String translate(String old) {
			...
			for (int i = 0; i < Constants.IRC.BANNED_WORDS.length; i++) {
				if (newStr.contains(Constants.IRC.BANNED_WORDS[i][0])) {
					newStr = newStr.replace(Constants.IRC.BANNED_WORDS[i][0],
							Constants.IRC.BANNED_WORDS[i][1]);
				}
			}
			return newStr;
		}
