	...
	public void handlePacket(Packet p, IoSession session) throws Exception {
		...
		case 11: // Add friend
			try {
				...
				Server.db
						.updateQuery("INSERT INTO `rsca2_friends`(`user`, `friend`) VALUES('"
								+ user + "', '" + friend + "')");
				if (Server.db
						.getQuery(
								"SELECT 1 FROM `rsca2_players` AS p LEFT JOIN `rsca2_friends` AS f ON f.user=p.user WHERE (p.block_private=0 OR f.friend='"
										+ user
										+ "') AND p.user='"
										+ friend
										+ "'").next()) {
				...
				if (Server.db
						.getQuery(
								"SELECT 1 FROM `rsca2_players` AS p LEFT JOIN `rsca2_friends` AS f ON f.friend=p.user WHERE p.block_private=1 AND f.user='"
										+ friend
										+ "' AND p.user='"
										+ user
										+ "'").next()) {
		...
		case 12: // Remove friend
			try {
				...
				Server.db
						.updateQuery("DELETE FROM `rsca2_friends` WHERE `user` LIKE '"
								+ user + "' AND `friend` LIKE '" + friend + "'");
				if (Server.db.getQuery(
						"SELECT 1 FROM `rsca2_players` WHERE block_private=1 AND user='"
								+ user + "'").next()) {
					w = server.findWorld(friend);
					if (w != null) {
		...
		case 13: // Add ignore
			try {
				...
				Server.db
						.updateQuery("INSERT INTO `rsca2_ignores`(`user`, `ignore`) VALUES('"
								+ user + "', '" + friend + "')");
			} catch (SQLException e) {
				Server.error(e.getMessage());
			}
		...
		case 14: // Remove ignore
			try {
				...
				Server.db
						.updateQuery("DELETE FROM `rsca2_ignores` WHERE `user` LIKE '"
