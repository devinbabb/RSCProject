...
public class PrivacySettingHandler implements PacketHandler {
	...
	public void handlePacket(Packet p, IoSession session) throws Exception {
		...
		case 0: // Chat block
			try {
				Server.db.updateQuery("UPDATE `rsca2_players` SET block_chat="
		...
		case 1: // Private block
			try {
				Server.db
						.updateQuery("UPDATE `rsca2_players` SET block_private="
								+ (on ? 1 : 0) + " WHERE user='" + user + "'");
				ResultSet result = Server.db
						.getQuery("SELECT user FROM `rsca2_friends` WHERE friend='"
								+ user
								+ "' AND user NOT IN (SELECT friend FROM `rsca2_friends` WHERE user='"
								+ user + "')");
				while (result.next()) {
		...
		case 2: // Trade block
			try {
				Server.db.updateQuery("UPDATE `rsca2_players` SET block_trade="
						+ (on ? 1 : 0) + " WHERE user='" + user + "'");
			} catch (SQLException e) {
				Server.error(e.getMessage());
			}
		...
		case 3: // Duel block
			try {
				Server.db.updateQuery("UPDATE `rsca2_players` SET block_duel="
						+ (on ? 1 : 0) + " WHERE user='" + user + "'");
			} catch (SQLException e) {
				Server.error(e.getMessage());
			}
