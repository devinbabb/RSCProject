...
public class PlayerLoginHandler implements PacketHandler {
	...
	public void handlePacket(Packet p, IoSession session) throws Exception {
		...
		if (loginCode == 0 || loginCode == 1 || loginCode == 99) {
			...
			try {
				Server.db
						.updateQuery("UPDATE `rsca2_players` SET online=1 WHERE user='"
								+ user + "'");
			} catch (Exception e) {
			}
	...
	private byte validatePlayer(long user, String pass, String ip) {
		...
		try {
			ResultSet result = Server.db
					.getQuery("SELECT r.pass, r.banned, r.owner, u.group_id, b.id AS b_id FROM `rsca2_players` AS r INNER JOIN `users` AS u ON u.id=r.owner LEFT JOIN `bans` AS b on (b.username LIKE u.username OR b.ip LIKE '"
							+ ip + "') WHERE `user`='" + user + "'");
			if (!result.next()) {
				return 2;
			}
