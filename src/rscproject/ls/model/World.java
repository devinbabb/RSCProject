...
import rscproject.ls.packetbuilder.loginserver.MiscPacketBuilder;
...
public class World {
	...
	public void registerPlayer(long user, String ip) {
		...
		try {
			result = Server.db
					.getQuery("SELECT owner, block_private FROM `rsca2_players` WHERE `user`='"
							+ user + "'");
			if (!result.next()) {
				return;
			}
			...
			result = Server.db
					.getQuery("SELECT user FROM `rsca2_friends` WHERE `friend`='"
							+ user
							+ "'"
							+ (blockPrivate ? " AND user IN (SELECT friend FROM `rsca2_friends` WHERE `user`='"
									+ user + "')"
									: ""));
			while (result.next()) {
			...
			Server.db
					.updateQuery("INSERT INTO `rsca2_logins`(`user`, `time`, `ip`) VALUES('"
							+ user + "', '" + now + "', '" + ip + "')");
			Server.db
					.updateQuery("UPDATE `rsca2_players` SET login_date=" + now
							+ ", login_ip='" + ip + "' WHERE user='" + user
							+ "'");
	...
	public void unregisterPlayer(long user) {
		...
		try {
			Server.db
					.updateQuery("UPDATE `rsca2_players` SET online=0 WHERE user='"
							+ user + "'");
		} catch (Exception e) {
			Server.error(e);
		}
