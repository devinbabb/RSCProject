...
/**
 * Handles saving players information.
 * 
 * @author Devin
 */
...
public class PlayerSave {
	...
	public static PlayerSave loadPlayer(long user) {
		...
		try {
			result = Server.db
					.getQuery("SELECT r.*, u.username AS owner_username, u.group_id as gid, u.sub_expires as subexp FROM `rsca2_players` AS r INNER JOIN `users` AS u ON u.id=r.owner WHERE `user`='"
							+ save.getUser() + "'");
			if (!result.next()) {
				return save;
			}
			...
			if (result.getInt("gid") == 8) {
				save.setOwner(sowner, Integer.parseInt("5"),
						result.getLong("subexp"));
			} else {
				save.setOwner(sowner, result.getInt("group_id"),
						result.getLong("subexp"));
			}
			...
			result = Server.db
					.getQuery("SELECT * FROM `rsca2_experience` WHERE `user`='"
							+ save.getUser() + "'");
			if (!result.next()) {
				return save;
			}
			...
			result = Server.db
					.getQuery("SELECT * FROM `rsca2_curstats` WHERE `user`='"
							+ save.getUser() + "'");
			if (!result.next()) {
				return save;
			}
			...
			result = Server.db
					.getQuery("SELECT id,amount,wielded FROM `rsca2_invitems` WHERE `user`='"
			...
			result = Server.db
					.getQuery("SELECT id,amount FROM `rsca2_bank` WHERE `user`='"
							+ save.getUser() + "' ORDER BY `slot` ASC");
			while (result.next()) {
			...
			result = Server.db
					.getQuery("SELECT friend FROM `rsca2_friends` WHERE `user`='"
							+ save.getUser() + "'");
			while (result.next()) {
			...
			result = Server.db
					.getQuery("SELECT `ignore` FROM `rsca2_ignores` WHERE `user`='"
							+ save.getUser() + "'");
			while (result.next()) {
				save.addIgnore(result.getLong("ignore"));
			}
			result = Server.db
					.getQuery("SELECT * FROM `rsca2_quests` WHERE `user`='"
							+ save.getUser() + "'");
			while (result.next()) {
	...
	public boolean save() {
		try {
			...
			Server.db.updateQuery("DELETE FROM `rsca2_bank` WHERE `user`='"
					+ user + "'");
			if (bankItems.size() > 0) {
			...
			Server.db.updateQuery("DELETE FROM `rsca2_invitems` WHERE `user`='"
					+ user + "'");

			ResultSet result = Server.db
					.getQuery("Select 1 FROM `rsca2_players` WHERE `user`='"
							+ user + "' AND `owner`='" + owner + "'");
			if (!result.next())
				return false;

			Server.db.updateQuery("UPDATE `rsca2_players` SET `combat`="
					+ combat + ", skill_total=" + skillTotal + ", `x`=" + x
					+ ", `y`='" + y + "', `fatigue`='" + fatigue
			...
			Server.db.updateQuery("DELETE FROM `rsca2_quests` WHERE `user`='"
					+ user + "'");
			...
			Server.db.updateQuery("UPDATE `rsca2_players` SET eventcd='"
