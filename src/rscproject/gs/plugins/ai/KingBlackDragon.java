...
public class KingBlackDragon extends NpcScript implements NpcAI {
	...
	public void absorbHealth(Npc npc) {
		for (Damager dam : npc.getSyndicate().getDamagers()) {
			...
			int drain = Formulae.Rand(
					(int) (dam.getPlayer().getMaxStat(3) * 0.25) / 2,
					(int) (dam.getPlayer().getMaxStat(3) * 0.25));
			...
			if (npc != null) {
				if (npc.getOpponent() != null
					...
					Instance.getDelayedEventHandler()
							.add(new rscproject.gs.event.MiniEvent((Player) npc
									.getOpponent(), 2500, new Object[] { npc }) {
								public void action() {
									Npc n = (Npc) super.args[0];
									if (n != null || n.getHits() > 0) {
										NpcScript script = new NpcScript();
										for (Damager dam : n.getSyndicate()
												.getDamagers()) {
											if (dam == null
													|| dam.getPlayer() == null)
												continue;
											...
											dam.getPlayer()
													...
													.sendMessage(
															"The "
																	+ n.getDef().name
																	+ " spits fire at you");
											GameObject obj = new GameObject(dam
											...
											Instance.getDelayedEventHandler()
													.add(new ObjectRemover(obj,
															500));
											Instance.getDelayedEventHandler()
													.add(new rscproject.gs.event.MiniEvent(
															(Player) n
																	.getOpponent(),
															2500,
															new Object[] { n }) {
