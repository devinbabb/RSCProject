...
public class KingBlackDragon extends NpcScript implements NpcAI {
	...
	try {
		...
		if (npc.getOpponent() != null && npc.getOpponent() instanceof Player) {
		    Instance.getDelayedEventHandler().add(new rscproject.gs.event.MiniEvent((Player) npc.getOpponent(), 2500, new Object[] { npc }) {
			public void action() {
				...
				for (Damager dam : n.getSyndicate().getDamagers()) {
				    ...
				    Instance.getDelayedEventHandler().add(new rscproject.gs.event.MiniEvent((Player) n.getOpponent(), 2500, new Object[] { n }) {
					public void action() {
