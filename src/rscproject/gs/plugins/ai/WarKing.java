		...
		if(war.isRunning()) {
			long now = System.currentTimeMillis();
   			if(now - war.getLastSpawn(n.getTeam()) > 30000) {
   				p.getActionSender().sendMessage("The queen summons a protector!");
   				war.sendMessage(n.getTeam(), "@whi@[@red@WAR@whi@]@red@ Your queen is under attack, its calling on monsters for backup!");
   				war.sendMessage(n.getTeam(), "Quickly go help your queen before the war is over!");
    	...
    	if(war.isRunning()) {
    		long now = System.currentTimeMillis();
   			if(now - war.getLastMessage(n.getTeam()) > 60000) {
   				war.sendMessage(n.getTeam(), "@whi@[@red@WAR@whi@]@red@ Your queen is under attack!");
   			}
      	}
