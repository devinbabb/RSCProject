		...
		if (newHp <= 0) {
			...
			if (attacker instanceof Player) {
				...
				if (attacker instanceof Player && opponent instanceof Npc) {
					...
					for (Damager fig : ((Npc) opponent).getSyndicate()
							.getDamagers()) {
						// boolean found = false;
						...
						if (fig.getPlayer() != null) {
							...
							if (fig.getPlayer() != attackerPlayer
									&& fig.isUseRanged()) {
								fig.getPlayer()
										.incExp(4, newXP * 4, true, true);
							...
							case 0:
								for (int x = 0; x < 2; x++) {
									fig.getPlayer()
											.incExp(x, newXP, true, true);
									fig.getPlayer().getActionSender()
											.sendStat(x);
								}
							...
							case 1:
								fig.getPlayer()
										.incExp(2, newXP * 3, true, true);
								fig.getPlayer().getActionSender().sendStat(2);
								break;
							case 2:
								fig.getPlayer()
										.incExp(0, newXP * 3, true, true);
								fig.getPlayer().getActionSender().sendStat(0);
								break;
							case 3:
								fig.getPlayer()
										.incExp(1, newXP * 3, true, true);
