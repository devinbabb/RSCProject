...
//		
	...
	if (cmd.equals("event")) {
        ...
        if (cmd.equals("eventweek")) {
            ...
            if (now - player.getEventCD() <= 3600) { player.getActionSender().sendMessage("You must wait an hout before using this command again."); return; }
            if (System.currentTimeMillis() - player.lastDeath < 150000) {
                player.getActionSender().sendMessage("You need to wait 150 seconds after you have died to use this again");
                return;
            }
	...
	if (cmd.equals("goto") || cmd.equals("summon")) {
        ...
        if (cmd.equals("pk")) {
                ...
                if(place+1 <= world.wildAttackable.length) {
                        ...
                        if(Integer.parseInt(args[1]) == 1) { System.out.println("This2"); world.wildAttackable[place] = true; System.out.println("This"); }
                        if(Integer.parseInt(args[1]) == 0) { world.wildAttackable[place] = false; }
