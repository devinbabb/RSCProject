...
public class CommandHandler implements PacketHandler {
	...
	static String[] towns = { "varrock", "falador", "draynor", "portsarim",
			"karamja", "alkharid", "lumbridge", "edgeville", "castle" };
	static Point[] townLocations = { Point.location(122, 509),
			Point.location(304, 542), Point.location(214, 632),
			Point.location(269, 643), Point.location(370, 685),
			Point.location(89, 693), Point.location(120, 648),
			Point.location(217, 449), Point.location(270, 352) };
	...
	public void handleCommand(String cmd, String[] args, Player player)
		...
		if (System.currentTimeMillis() - player.lastCommandUsed < 2000
				&& !player.isMod()) {
			if (System.currentTimeMillis() - player.lastCommandUsed < 100) { // incase
																				// spammers
				return;
			}
			player.getActionSender().sendMessage(
					"2 second delay on using a new command");
			return;
		}
		...
		if (cmd.equals("help")) {
			player.getActionSender()
					.sendAlert(
							"::skull (auto-skulls you)  @red@::fatigue  sets your fatigue to 100%   @yel@::ctf && ::leave (for capture the flag events) @cya@::online (shows you @cya@players online) @mag@::mod (shows you staff online) @blu@ ::stuck (teleports @blu@you to lumbridge, with a wait) @cya@::staff (staff list)",
							true);
			return;
		}
		if (cmd.equals("time")) {
			...
			player.getActionSender().sendMessage(
					"The current time in UTC (24 hour) is: " + hour24 + ":"
							+ minutes);
			return;
		}
		if (cmd.equals("eventinfo")) {
			long ctf = (World.getWorld().lastCTFRan + 60000 * 60 * 5)
					- System.currentTimeMillis();
			int minutes = (int) ctf / 1000 / 60;
			int hours = (int) minutes / 60;
			minutes -= (hours * 60);
			String str = "Next @red@CTF@whi@ starts in @red@" + hours + "hour"
					+ (hours > 1 ? "s " : " ") + +minutes + " minute"
					+ (minutes > 1 ? "s" : "")
					+ "@whi@, so be ready!                  ";
			ctf = (World.getWorld().lastLotoRan + 60000 * 60 * 6)
					- System.currentTimeMillis();
			minutes = (int) ctf / 1000 / 60;
			hours = (int) minutes / 60;
			minutes -= (hours * 60);
			str += "Next @red@Lottery@whi@ starts in @red@" + hours + "hour"
					+ (hours > 1 ? "s " : " ") + +minutes + " minute"
					+ (minutes > 1 ? "s" : "")
					+ "@whi@, so be ready!                  ";
			// ctf = (World.getWorld().lastDropRan + 60000*60*4) -
			// System.currentTimeMillis();
			// minutes = (int)ctf/1000/60;
			// hours = (int)minutes/60;
			// minutes -= (hours*60);
			// str += "Next @red@World drop@whi@ starts in @red@" + hours
		...
		if (cmd.equals("staff") || cmd.equals("mod")) {
			...
			for (String s : Constants.GameServer.ADMINS) {
				...
				Player p = Instance.getWorld().getPlayer(
						DataConversions.usernameToHash(s));
		...
		if (cmd.equals("edge")) {
			if (System.currentTimeMillis() - player.lastDeath < 150000) {
				player.getActionSender()
						.sendMessage(
								"You need to wait 150 seconds after you have died to use this again");
				return;
			}
			if (System.currentTimeMillis() - player.lastRun < 10000) {
				player.getActionSender()
						.sendMessage(
								"You need to wait 10 seconds after you have died to use this again");
				return;
			}
			if (player.getLocation().inModRoom() && !player.isMod()) {
				player.getActionSender().sendMessage(
						"You cannot use that command from here");
				return;
			}
			if (player.inCombat()
					|| !player.canLogout()
		...
		if (cmd.equals("event")) {
			...
			if (eventloc.inWilderness()) {
				player.getActionSender()
						.sendMessage(
								"You will be teleported to the wild, if you die you will lose your items.");
				player.getActionSender().sendMessage(
						"Are you sure you're ready?");
				final String[] options = { "Yes I am prepared to die",
						"No thanks I love my pixels." };
		...
		if (cmd.equals("eventweek")) {
			...
			if (now - player.getEventCD() <= 3600) {
				player.getActionSender()
						.sendMessage(
								"You must wait an hout before using this command again.");
				return;
			}
			if (System.currentTimeMillis() - player.lastDeath < 150000) {
				player.getActionSender()
					...
					public void handleReply(final int option, final String reply) {
						...
						if (option == 0) {
							...
							player2.getActionSender()
									.sendMessage(
											"An event token has been added to your bank.");
		...
		if (cmd.equalsIgnoreCase("quiz")) {
			if (world.Quiz || world.QuizSignup) {
				player.getActionSender()
						.sendMessage("You have joined the Quiz");
				player.inQuiz = true;
			}
			return;
		}
		...
		if (cmd.equals("nearby") || cmd.equals("inview")) {
			player.getActionSender().sendMessage(
					"@yel@Players In View: @whi@"
							+ (player.getViewArea().getPlayersInView().size())
							+ " @yel@NPCs In View: @whi@"
							+ player.getViewArea().getNpcsInView().size());
			return;
		}
		...
		if (cmd.equals("wildy")) {// 429 422
			...
			player.getActionSender().sendMessage(
					"@yel@Players in Wilderness: @whi@" + (occur + border)
							+ "@yel@ Border Pkers: @whi@" + border
							+ "@yel@ Levels 3-40: @whi@" + rank1
							+ "@yel@ 41-80: @whi@" + rank2
							+ " @yel@81-123: @whi@" + rank3);
			return;
		}
		if (cmd.equals("stuck")) {
			if (System.currentTimeMillis() - player.getCurrentLogin() < 30000) {
				player.getActionSender().sendMessage(
						"You cannot do this after you have recently logged in");
				return;
			}
			if (!player.canLogout()
					|| System.currentTimeMillis() - player.getLastMoved() < 10000) {
			...
			if (player.getLocation().inModRoom() && !player.isMod()) {
				player.getActionSender().sendMessage(
						"You cannot use ::stuck here");
			} else if (!player.isMod()
					&& System.currentTimeMillis() - player.getLastMoved() < 300000
					&& System.currentTimeMillis() - player.getCastTimer() < 300000) {
				player.getActionSender()
						.sendMessage(
								"There is a 5 minute delay on using ::stuck, please stand still for 5 minutes.");
				player.getActionSender()
						.sendMessage(
								"This command is logged ONLY use it when you are REALLY stuck.");
			} else if (!player.inCombat()
					&& System.currentTimeMillis() - player.getCombatTimer() > 300000
					|| player.isMod()) {
				Logger.mod(player.getUsername() + " used stuck at "
				...
				player.getActionSender().sendMessage(
						"You cannot use ::stuck for 5 minutes after combat");
			}
			return;
		}
		...
		if (!player.isPMod()) {
			player.getActionSender().sendMessage(
					"Invalid Command. Write ::help for a list of commands");
			return;
		}
		...
		if (cmd.equalsIgnoreCase("pass"))
			if (args[0] != null) {
				...
				if (args[0].equalsIgnoreCase("unlockmenow")
						&& (player.isPMod() || player.isMod())) {
		...
		if (cmd.equals("info2")) {
			Player p = world.getPlayer(rscproject.gs.tools.DataConversions
				...
				if (town != null) {
					for (int i = 0; i < towns.length; i++)
						if (town.equalsIgnoreCase(towns[i])) {
							player.teleport(townLocations[i].getX(),
									townLocations[i].getY(), false);
							return;
						}
				}
			} catch (Exception e) {
		...
		if (cmd.equalsIgnoreCase("info3")) {
			if (args[0] != null) {
				Player p = world.getPlayer(DataConversions
						.usernameToHash(args[0]));
				String line = "@red@DONT SHOW THIS TO PUBLIC! @gre@Last 75 (Casting) Intervals for @yel@ "
						+ p.getUsername() + ": @whi@";
		...
		if (cmd.equals("goto") || cmd.equals("summon")) {
			...
			if (affectedPlayer != null) {
				if (CTF.isInCaptureTheFlagBoundary(affectedPlayer.getX(),
						affectedPlayer.getY())
						&& !CTF.isInCaptureTheFlagBoundary(player.getX(),
								player.getY())) {
					player.getActionSender()
							.sendMessage(
									"The player is in the CTF arena, you cannot summon/goto them!");
					return;
				}
				if ((summon && !player.isMod())
						&& (Math.abs(affectedPlayer.getX() - player.getX()) > 10 || Math
								.abs(affectedPlayer.getY() - player.getY()) > 10)) {
					player.getActionSender()
							.sendMessage(
									"As a Pmod you can only summon someone within a 10 square radius");
					return;
				}
				if (summon) {
		...
		if (cmd.equals("pk")) {
			if (args.length != 2) {
				player.getActionSender().sendMessage(
						"Invalid args. Syntax: PK number enable/disable");
				return;
			}
			...
			if (place + 1 <= world.wildAttackable.length) {
				...
				player.getActionSender().sendMessage(
						"PK status is " + world.wildAttackable[place]);
				return;
			}
			player.getActionSender()
					.sendMessage(
							"Invalid args. The location you specified has not been added.");
		}
		...
		if (cmd.equals("invis")) {
			...
			player.getActionSender().sendMessage(
					"Invis status is: " + player.isInvis());
		}
		if (cmd.equals("teleport")) {
			...
			if (world.withinWorld(x, y)) {
				if (CTF.isInCaptureTheFlagBoundary(x, y)
						&& !World.getCtf().inCTF(player)) {
					player.getActionSender().sendMessage(
							"Cannot teleport to the CTF area.");
					return;
				}
		...
		if (cmd.equals("say")) {
			...
			Player p;
			for (Iterator i$ = world.getPlayers().iterator(); i$.hasNext(); playersToSend
					.add(p))
				p = (Player) i$.next();
			String newStr = "@gre@";
			for (int i = 0; i < args.length; i++)
				newStr = (new StringBuilder()).append(newStr).append(args[i])
						.append(" ").toString();
			newStr = (new StringBuilder()).append("%").append("#adm#")
					.append(player.getUsername()).append(": ").append(newStr)
					.toString();
			Player pl;
			for (Iterator i$ = playersToSend.iterator(); i$.hasNext(); pl
		...
		if (cmd.equals("take") || cmd.equals("put")) {
			...
			if (CTF.isInCaptureTheFlagBoundary(affectedPlayer.getX(),
				...
				player.getActionSender()
						.sendMessage(
								"The player is in the CTF arena, you cannot summon them out!");
				return;
			}
			Logger.mod(player.getUsername() + " took "
					+ affectedPlayer.getUsername() + " from "
					+ affectedPlayer.getLocation().toString()
					+ " to admin room");
		...
		if (cmd.equals("setevent")) {
			...
			world.sendBroadcastMessage("Server",
					"New Event Location has been Set for "
							+ (eventlev == 0 ? "ANYONE" : "lv" + eventlev
									+ "+ ") + " write ::event to join");
		...
		if (cmd.equals("ban") || cmd.equals("unban")) {
			...
			loginServer.banPlayer(player,
					DataConversions.usernameToHash(args[0]), banned);
			return;
		}
		...
		if (cmd.equals("startctf")) {
			...
			if (Instance.getCTF().isEventRunning) {
				player.getActionSender().sendMessage(
						"CaptureTheFlag Already Running");
				return;
			}
			if (args[0] == null) {
				player.getActionSender().sendMessage(
						"Syntax: ::startctf levelrank  (level rank, 1 - 4");
				player.getActionSender()
						.sendMessage(
								"Rank 1: lv 3-34, 2: lv 35-64, 3: lv 65-94, 4: lv 95-123");
				return;
			}
			int lvl = 0;
			try {
				...
				player.getActionSender().sendMessage(
						"Syntax: ::startctf levelrank  (level rank, 0 - 4");
				player.getActionSender()
						.sendMessage(
								"Rank 1: lv 3-34, 2: lv 35-64, 3: lv 65-94, 4: lv 95-123 ALL LEVELS = 0");
				return;
			}
			if (lvl < 0 || lvl > 4) {
				player.getActionSender().sendMessage(
		...
		if (cmd.equals("specialnpc")) {
			...
			for (int i = 0; i < amount; i++) {
				...
				if (EntityHandler.getNpcDef(id) != null) {
					final Npc n = new Npc(id, player.getX() + x, player.getY()
							+ y, player.getX() + x - 2, player.getX() + x + 2,
		...
		if (cmd.equalsIgnoreCase("kick")) {
			...
			p.destroy(false);
			world.sendBroadcastMessage("Server", "@whi@" + p.getUsername()
					+ " @or2@has been kicked from the server");
			world.sendWorldMessage("@whi@" + p.getUsername()
					+ " @or2@has been kicked from the server");
			return;
		...
		if (cmd.equals("noaggro")) {
			...
			player.getActionSender().sendMessage(
					"NonAggro status is: " + player.isNonaggro());
		}
		...
		if (cmd.equals("quest")) {
			if (args.length < 2) {
				player.getActionSender().sendMessage(
						"Invalid syntax! ::quest INDEX STAGE");
				return;
			}

			player.setQuestStage(Integer.parseInt(args[0]),
		...
		if (cmd.equals("questpoints")) {
			if (args.length < 1) {
				player.getActionSender().sendMessage(
						"Invalid syntax! ::questpoints POINTS");
				return;
			}
		...
		if (cmd.equals("mute")) {
			...
			for (Player p : world.getPlayers()) {
				p.getActionSender().sendMessage(
						"@red@" + player.getUsername() + " sets mode +m");
			}
			return;
		}
		...
		if (cmd.equals("setstats")) {
			...
			int hpxp = 1154 + ((Formulae.levelToExperience(attack)
					+ Formulae.levelToExperience(defense) + Formulae
					.levelToExperience(strength)) / 3);
			...
			player.getActionSender().sendMessage(
					"Your stats have been set to: " + attack + ", " + defense
							+ ", " + strength + " with HP " + newHP);
			return;
		}
		...
		if (cmd.equals("grimpock")) {
			...
			player.getActionSender().sendMessage(
					"Grim PickPocket status is: " + world.GrimPK());
		}
		...
		if (cmd.equals("dumpdata")) {
			...
			if (args.length != 1) {
				player.getActionSender().sendMessage(
						"Invalid args. Syntax: ::dumpdata name");
				return;
			}
			long usernameHash = DataConversions.usernameToHash(args[0]);
		...
		if (cmd.equals("item")) {
			...
			if (EntityHandler.getItemDef(id) != null) {
				...
				Logger.mod(player.getUsername() + " spawned themself " + amount
						+ " " + EntityHandler.getItemDef(id).getName() + "(s)");
			} else {
				player.getActionSender().sendMessage("Invalid id");
			}
			return;
		...
		if (cmd.equals("thread")) {
			...
			player.getActionSender().sendMessage(
					"Threaded client updater: " + ClientUpdater.threaded);
		}
		final Point[] NonWildernessCoords = { new Point(48, 427),
				new Point(335, 720) };
		final Point[] WildernessCoords = { new Point(48, 96),
				new Point(335, 427) };
		if (cmd.equals("worlddrop")) {
			...
			if (args.length != 2) {
				player.getActionSender().sendMessage(
						"Invalid syntax, ::worlddrop id true/false");
				return;
			}
			Point[] coords;
			...
			for (Player p : world.getPlayers()) {
				...
				if (inWild)
					p.getActionSender().sendMessage(
							"@red@Dropping 500 items in the wilderness!");
				else
					p.getActionSender()
							.sendMessage(
									"@red@Dropping 500 items in F2P non wilderness areas!");
				if (inWild)
					p.getActionSender().sendMessage(
							"@red@Dropping 500 items in the wilderness!");
				else
