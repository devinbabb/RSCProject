...
import rscproject.gs.event.WalkToPointEvent;
...
import rscproject.gs.model.Projectile;
...
public class SpellHandler implements PacketHandler {
	...
	Instance.getDelayedEventHandler().add(new WalkToMobEvent(player, affectedMob, 5) {
		...
		default:
			...
			Player affectedPlayer = (Player) affectedMob;
			if(Instance.getCTF().isSameTeam(affectedPlayer, owner))
			    return;
			...
			if (owner instanceof Player && !Instance.getCTF().inCTF(owner)) {
			  ...
			  Player toLoot = owner;
				...
				for (Damager fig : ((Npc) affectedMob).getSyndicate().getDamagers()) {
				    ...
				    if (fig.isUseMagic() && !fig.isUseCombat())
					continue;
				    if (Instance.getCTF().inCTF(fig.getPlayer()))
					continue;
				    ...
				    if (fig.getPlayer() != null) {
					newXP = (exp * fig.getDamage()) / npc.getDef().hits;
					if (CTF.isInCaptureTheFlagBoundary(fig.getPlayer().getX(), fig.getPlayer().getY()))
					    newXP = 0;
