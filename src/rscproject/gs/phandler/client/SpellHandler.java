...
import rscproject.gs.event.*;
import rscproject.gs.external.EntityHandler;
import rscproject.gs.external.ItemSmeltingDef;
import rscproject.gs.external.ReqOreDef;
import rscproject.gs.external.SpellDef;
import rscproject.gs.model.*;
import rscproject.gs.model.mini.Damager;
	...
	private void handleMobCast(final Player player, Mob affectedMob,
		...
		if (affectedMob instanceof Player) {
			...
			if (player.getLocation().inWilderness()
					&& System.currentTimeMillis() - other.lastRun < 1000) {
				player.resetPath();
				return;
			}
		}
		if (player.getLocation().inWilderness()
				&& System.currentTimeMillis() - player.lastRun < 3000) {
			player.resetPath();
			return;
		}
				...
				new WalkToMobEvent(player, affectedMob, 5) {
					public void arrived() {
						...
						if (affectedMob instanceof Npc) {
							if (World.getWar().isMyQueen((Npc) affectedMob,
									owner)) {
								owner.getActionSender().sendMessage(
										"You cannot attack your own king!");
								return;
							}
							if (!World.getWar().inATeam(owner)
									&& World.getWar().isNpcAssociated(
											(Npc) affectedMob)) {
								owner.getActionSender()
										.sendMessage(
												"You cannot attack a king! Join a team, be apart of the war!");
								return;
							}
						}
							...
							if (newp <= 0) {
								...
								if (owner instanceof Player) {
									int exp = DataConversions.roundUp(Formulae
											.combatExperience(affectedMob) / 4D);
									int newXP = 0;
									if (owner instanceof Player
											&& affectedMob instanceof Npc) {
										Npc npc = (Npc) affectedMob;

										for (Damager fig : ((Npc) affectedMob)
												.getSyndicate().getDamagers()) {
											if (fig.isUseMagic()
													&& !fig.isUseCombat())
												continue;
											...
											if (fig.getPlayer() != null) {
												newXP = (exp * fig.getDamage())
														/ npc.getDef().hits;

												if (fig.isUseRanged()) {
													fig.getPlayer().incExp(4,
															newXP * 3, true);
													fig.getPlayer()
															.getActionSender()
															.sendStat(4);
									...
									if (owner instanceof Player
										...
										for (Damager fig : ((Npc) affectedMob)
											...
											if (fig.isUseMagic()
													&& !fig.isUseCombat())
												continue;
											if (Instance.getCTF().inCTF(
													fig.getPlayer()))
												continue;
											// if(fig.player != attackerPlayer)
											/*
											 * for(Player p :
											 * attackerPlayer.getViewArea
											 * ().getPlayersInView()) if(p ==
											 * fig.player) found = true;
											 * if(!found) // skip a person who
											 * is not in the area. continue;
											 */
											...
											if (fig.getPlayer() != null) {
												newXP = (exp * fig.getDamage())
														/ npc.getDef().hits;
												if (CTF.isInCaptureTheFlagBoundary(
														fig.getPlayer().getX(),
														fig.getPlayer().getY()))
													newXP = 0;
