...
public class ObjectAction implements PacketHandler {
	...
	public void handlePacket(Packet p, IoSession session) {
		 ...
		 * System.out.println("[Anti-Bot] " + player.getUsername() +
				...
				new WalkToObjectEvent(player, object, false) {
					public void arrived() {
						...
						try {
							...
							if (object.getID() == 1005
									&& Instance.getCTF().isEventRunning
									&& CTF.isInCaptureTheFlagBoundary(
											owner.getX(), owner.getY())) {
								Instance.getCTF()
										.handleOpenChest(owner, object);
								return;
							}
							if (command.equals("steal")
									&& (object.getID() == CTF.BLUE_FLAG_ID || object
											.getID() == CTF.RED_FLAG_ID)) {
								if (Instance.getCTF().isEventRunning
										&& CTF.isInCaptureTheFlagBoundary(
												owner.getX(), owner.getY()))
									Instance.getCTF().handleFlagSteal(owner,
											object);
								return;
							}
							if (command.equals("capture")
								...
								if (Instance.getCTF().isEventRunning
										&& CTF.isInCaptureTheFlagBoundary(
												owner.getX(), owner.getY()))
									Instance.getCTF().handleFlagScore(owner,
											object);
								return;
							}
							// Nothing interesting
									...
									&& object.containsItem() == 29) // hopper
							{
								...
								Instance.getDelayedEventHandler().add(
										new rscproject.gs.event.MiniEvent(
												owner, 1000) {
											public void action() {
												owner.getActionSender()
														.sendMessage(
