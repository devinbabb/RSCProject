...
import rscproject.gs.event.WalkToObjectEvent;
import rscproject.gs.external.AgilityCourseDef;
import rscproject.gs.external.AgilityDef;
import rscproject.gs.external.EntityHandler;
import rscproject.gs.external.GameObjectDef;
import rscproject.gs.external.ObjectFishDef;
import rscproject.gs.external.ObjectFishingDef;
import rscproject.gs.external.ObjectWoodcuttingDef;
...
import rscproject.gs.model.Point;
...
public class ObjectAction implements PacketHandler {
	...
	Instance.getDelayedEventHandler().add(new WalkToObjectEvent(player, object, false) {
		...
		try {
		    ...
		    if (object.getID() == 1005 && Instance.getCTF().isEventRunning && CTF.isInCaptureTheFlagBoundary(owner.getX(), owner.getY())) {
		    	Instance.getCTF().handleOpenChest(owner, object);
		    	return;
		    }
		    if (command.equals("steal") && (object.getID() == CTF.BLUE_FLAG_ID || object.getID() == CTF.RED_FLAG_ID)) {
				if (Instance.getCTF().isEventRunning && CTF.isInCaptureTheFlagBoundary(owner.getX(), owner.getY()))
					Instance.getCTF().handleFlagSteal(owner, object);
					return;
			    }
		    if (command.equals("capture") && (object.getID() == CTF.BLUE_FLAG_ID || object.getID() == CTF.RED_FLAG_ID)) {
				if (Instance.getCTF().isEventRunning && CTF.isInCaptureTheFlagBoundary(owner.getX(), owner.getY()))
					Instance.getCTF().handleFlagScore(owner, object);
					return;
			    }
		    // Nothing interesting
			...
			Instance.getDelayedEventHandler().add(new rscproject.gs.event.MiniEvent(owner, 1000) {
			    public void action() {
				owner.getActionSender().sendMessage("The grain slides down the chute");
			    }
			});
			// Konijn/xEnt/Devin == TEAM WORK BIATCH.
