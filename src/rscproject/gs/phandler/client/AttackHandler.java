...
public class AttackHandler implements PacketHandler {
	...
	public void handlePacket(Packet p, IoSession session) throws Exception {
		...
		if (affectedMob instanceof Player) {
			...
			if (CTF.isInCaptureTheFlagBoundary(pl.getX(), pl.getY())
					&& !Instance.getCTF().isEventRunning) {
				pl.teleport(100, 500, false);
				return;
			}
			if (Instance.getCTF().isSameTeam(pl, player)) {
				player.getActionSender().sendMessage(
						"You cannot attack your own team member");
				return;
			}
		}
		if (affectedMob instanceof Npc) {
			...
			if (World.getWar().inSameTeam(player, (Npc) affectedMob)) {
				(player).getActionSender().sendMessage(
						"This monster is protecting your queen!");
				return;
			}
		}
		...
		if (affectedMob instanceof Player) {
			...
			if (pl.getLocation().inWilderness()
					&& System.currentTimeMillis() - pl.lastRun < 3000) {
				return;
			}
		}
						...
						public void arrived() {
							...
							if (affectedMob instanceof Npc) {
								if (((Npc) affectedMob).isScripted()) {
									if (World.getWar().isMyQueen(
											(Npc) affectedMob, owner)) {
										owner.getActionSender()
												.sendMessage(
									...
									System.out.println(World.getWar().inATeam(
											owner)
											+ " && "
						...
						public void arrived() {
							...
							if (affectedMob instanceof Npc) {
								if (((Npc) affectedMob).isScripted()) {
									...
									// Instance.getPluginHandler().getNpcAIHandler(affectedMob.getID()).onRangedAttack(owner,
									// (Npc) affectedMob);
								}
							}
							if (affectedMob.getID() == 35) {
