...
public class PickupItem implements PacketHandler {
	...
	if(!item.getDef().canTrade()) {
		if(item.droppedby() != 0 && rscproject.gs.tools.DataConversions.usernameToHash(player.getUsername()) != item.droppedby()) {
	...
	Instance.getDelayedEventHandler().add(new WalkToPointEvent(player, location, 1, false) {
		...
		if (pickedUpRare) {
		    ...
		    final long playerLong = rscproject.gs.tools.DataConversions.usernameToHash(owner.getUsername());
