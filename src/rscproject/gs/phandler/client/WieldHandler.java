...
public class WieldHandler implements PacketHandler {
	...
	public void handlePacket(Packet p, IoSession session) throws Exception {
		...
		if (player.getLocation().inWilderness()
				&& item.getDef().isMembers()
				&& Constants.GameServer.F2P_WILDY
				&& !CTF.isInCaptureTheFlagBoundary(player.getX(), player.getY())) {
			player.getActionSender().sendMessage(
		...
		if (Instance.getCTF().inCTF(player))
			if (item.wieldingAffectsItem(CTF.BLUE_GEAR_CHEST)
					|| item.wieldingAffectsItem(CTF.BLUE_GEAR_LEG)) {
				player.getActionSender()
						.sendMessage(
								"You may not equip this body part in CaptureTheFlag, ::leave then rejoin to do that.");
				return;
			}
		...
		if (World.getWar().inATeam(player)) {
			if (item.wieldingAffectsItem(GlobalWar.RED_CAPE)
					|| item.wieldingAffectsItem(GlobalWar.BLUE_CAPE)) {
	...
	private void wieldItem(Player player, InvItem item) {
		...
		player.getActionSender().sendSound("click");
		if (Instance.getCTF().inCTF(player)
				&& (item.wieldingAffectsItem(CTF.BLUE_GEAR_CHEST) || item
						.wieldingAffectsItem(CTF.BLUE_GEAR_LEG)))
			return;
