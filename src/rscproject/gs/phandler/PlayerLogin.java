...
public class PlayerLogin implements PacketHandler {
	...
	if (loginCode == 0 || loginCode == 1 || loginCode == 99) {
	    ...
	    if(World.getCtf().isEventRunning) {
	    	sender.sendMessage("%#adm#" + "@yel@Server@whi@" + ": @gre@" + "@gre@Capture the Flag@whi@ is running! (Levels " + CTF.getLevelString(CTF.currentLevelRank) + ")");
	    ...
	    //player.getActionSender().sendAlert("Happy Birthday RSCAngel!!! XP rates are TIMES TWO for today only. Check our website/twitter for more info.", false);
