...
public class PlayerLogin implements PacketHandler {
	...
	public void handlePacket(rscproject.gs.connection.Packet p,
		...
		if (loginCode == 0 || loginCode == 1 || loginCode == 99) {
			...
			rscproject.gs.builders.MiscPacketBuilder sender = player
					.getActionSender();
			...
			if (World.getCtf().isEventRunning) {
				sender.sendMessage("%#adm#" + "@yel@Server@whi@" + ": @gre@"
			...
			// player.getActionSender().sendAlert("Happy Birthday RSCAngel!!! XP rates are TIMES TWO for today only. Check our website/twitter for more info.",
			// false);
