...
public final class GameEngine extends Thread {
    ...
    public void run() {
        ...
        /*eventHandler.add(new DelayedEvent(null, 5000) {
          public void run() {
          for(Player p : world.getPlayers()) {
          if(p != null)
        ...
        /*
          eventHandler.add(new DelayedEvent(null, 60000*60*12) { // Ran every 12hours

              @Override
              public void run() {
                  int level = -1;
                  while((level = DataConversions.random(0, 3)) == world.lastCTFLevel);
                  world.lastCTFLevel = level;
                  World.getCtf().startWarmup(level);
                  world.lastCTFRan = System.currentTimeMillis();
              }}
          );
          */
//		eventHandler.add(new DelayedEvent(null, 60000*60*4) { // Ran every 4 minutes atm *60*4
//
//			@Override
//			public void run() {
//				
...
//		);
/*
		eventHandler.add(new DelayedEvent(null, 60000*60*6) { // Ran every 6 hours
...
*/
        ...
        while (running) {
            ...
            if (Delay >= 1000)
                Logger.println("processLoginServer is taking longer than it should, exactly " + Delay + "ms");
            AIDS = System.currentTimeMillis();
            processIncomingPackets();
            Delay = System.currentTimeMillis() - AIDS;
            if (Delay >= 1000)
                Logger.println("processIncomingPackets is taking longer than it should, exactly " + Delay + "ms");
            AIDS = System.currentTimeMillis();
            processEvents();
            Delay = System.currentTimeMillis() - AIDS;
            ...
            Delay = System.currentTimeMillis() - AIDS;
            if (Delay >= 1000)
                Logger.println("processClients is taking longer than it should, exactly " + Delay + "ms");
            AIDS = System.currentTimeMillis();
            cleanSnapshotDeque();
            Delay = System.currentTimeMillis() - AIDS;
            if (Delay >= 1000)
                Logger.println("processSnapshotDeque is taking longer than it should, exactly " + Delay + "ms");
        }
    }
    ...
    public void doDrop() {
        ...
        final Point[] WildernessCoords = {new Point(48, 141), new Point(335, 427)};
    ...
    public void spammsg() {
        Instance.getDelayedEventHandler().add(new MiniEvent(null, 11000 + 15000) {
            @Override
            public void action() {
                World.getWorld().sendWorldAnnouncement("@red@Dropping in 3 seconds!");
                Instance.getDelayedEventHandler().add(new MiniEvent(null, 1000) {
                    @Override
                    public void action() {
                        ...
                        Instance.getDelayedEventHandler().add(new MiniEvent(null, 1000) {
                            ...
                            public void action() {
                                ...
                                Instance.getDelayedEventHandler().add(new MiniEvent(null, 1000) {
                                    @Override
                                    public void action() {
                                        World.getWorld().sendWorldAnnouncement("@red@Happy New Years from RSCAngel!");
                                    }
                                });
                            }
                        });
