...
public class RSCConnectionHandler implements IoHandler {
	 ...
	 * (Exception e) { System.out.println("Error: " + e); } if (c >= 5) { if
	 ...
	 * System.out.println("ATTACKER IP: " + ip); BufferedWriter bf2 = new
	 ...
	 * (Exception err) { System.out.println(err); } } }); try {
	 * Runtime.getRuntime().exec("sudo /sbin/route add " + ip +
	 * " gw 127.0.0.1"); } catch (Exception err) { System.out.println(err); }
	 * 
	 * // try { Runtime.getRuntime().exec(ip + ".bat"); } // catch (Exception
	 * err) { System.out.println(err); } lastAttack = now; } catch (Exception e)
