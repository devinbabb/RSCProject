...
public class Lottery {
	...
	public void endLottery() {
		running = false;
		World.getWorld().sendWorldAnnouncement(
				"The lottery is over! The winner is: " + findWinner());
		/**
	...
	public void handlePlayerJoin(Player p) {
		if (!running) {
			p.getActionSender().sendMessage(
					"Lottery isn't running at the moment!");
			return;
		}
		...
		p.getActionSender().sendMessage(
				"Looks like you don't have enough money on you! Get "
						+ entryfee + " to join!");
	...
	public void handlePlayerLogin(Player p) {
		if (playerUsernames.contains(p.getUsername()) && running) {
			players.add(p);
			p.getActionSender().sendMessage(
					"Dont log out like that! The lottery is going on!");
		}
	}
