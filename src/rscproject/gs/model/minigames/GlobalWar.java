...
public class GlobalWar {
	 ...
	 */
	private int[][] wildLocations = { { 219, 231 }, { 247, 339 }, { 237, 231 },
			{ 265, 339 } };

	/**
	 * Returns cordinates to where the base is (factors in player team + if
	 * event is low or high level)
	 * 
	 * @param p
	 *            player wishing to teleport.
	 * @return
	 */
	...
	/**
	 * Determines were the war is beind held, if true the war will be held at
	 * wild 15, otherwise wild 30.
	 */
	private boolean low = false;
	/**
	 * Determines when the last message to the red team was sent (warnings about
	 * queens)
	 */
	private long lastRedMessage = 0;
	/**
	 * Determines when the last message to the blue team was sent (warnings
	 * about queens)
	 */
	private long lastBlueMessage = 0;

	/**
	 * Determines when the last spawn to the red team was done (protection for
	 * queens)
	 */
	private long lastRedSpawn = 0;
	/**
	 * Determines when the last spawn to the blue team was done (protection for
	 * queens)
	 */
	...
	public boolean isNpcAssociated(Npc n) {
		...
		if (n.getID() == 415 || n.getID() == 18)
			return true;
		if (n.getID() == queenId && n.getX() == getStartCoords(0)[0]
				&& n.getY() == getStartCoords(0)[1]) {
			return true;
		} else if (n.getID() == queenId && n.getX() == getStartCoords(1)[0]
				&& n.getY() == getStartCoords(1)[1]) {
			return true;
		}
		return false;
	}
	...
	public boolean isMyQueen(Npc n, Player p) {
		if (!isRunning())
			return false;
		if (inRed(p) && n.getTeam() == 0 && n.getID() == queenId
				&& n.getX() == getStartCoords(0)[0]
				&& n.getY() == getStartCoords(0)[1]) {
			return true;
		} else if (inBlue(p) && n.getTeam() == 1 && n.getID() == queenId
				&& n.getX() == getStartCoords(1)[0]
				&& n.getY() == getStartCoords(1)[1]) {
			return true;
		}
		return false;
	 ...
	 * 
	 * @param low
	 *            if true the war will be held at wild 15, otherwise wild 30
	 */
	public void startWar(boolean low) {
		...
		for (Player p : world.getPlayers()) {
			p.getActionSender().sendMessage(
					"@red@Global war mode enabled! RSCAngel is at war!");
			p.getActionSender()
					.sendMessage(
							"@red@Go to edgeville to join a team and protect your queen! Or go PK solo!");
		}
		/**
		 * Spawn kings
		 */
		redQueen = new Npc(queenId, getStartCoords(0)[0], getStartCoords(0)[1],
				getStartCoords(0)[0] - 2, getStartCoords(0)[0] + 2,
				getStartCoords(0)[1] - 2, getStartCoords(0)[1] + 2);
		...
		blueQueen = new Npc(queenId, getStartCoords(1)[0],
				getStartCoords(1)[1], getStartCoords(1)[0] - 2,
				getStartCoords(1)[0] + 2, getStartCoords(1)[1] - 2,
				getStartCoords(1)[1] + 2);
		blueQueen.setTeam(1);
		 ...
		 */
		for (int i = 0; i < 5; i++) {
			Npc perc = new Npc(415, 215 - i, 436, 215 - 2 - i, 215 + 2 - i,
					436 - 2, 436 + 2);
			Npc rov = new Npc(18, 217 - i, 438, 217 - 2 - i, 217 + 2 - i,
					438 - 2, 438 + 2);
	...
	/**
	 * Starts the global advertisement for war (every 60s announces the war
	 * state)
	 */
			...
			public void run() {
				if (World.getWar().isRunning()) {
					for (Player p : world.getPlayers()) {
						...
						p.getActionSender().sendMessage(
								"@red@The red queen has @whi@"
										+ redQueen.getHits() + "@red@/@whi@"
										+ kingHits + "@red@ HP left");
						p.getActionSender().sendMessage(
								"@blu@The blue queen has @whi@"
										+ blueQueen.getHits() + "@blu@/@whi@"
										+ kingHits + "@blu@ HP left");
					}
				} else
					this.stop();
			}
	...
	public void endWar(int team) {
		if (team == 0) {
			for (Player p : world.getPlayers()) {
				p.getActionSender()
						.sendMessage(
								"@red@Red team is victorious and the war is done! RSCAngel is at peace!");
				handleCapeUnWield(p);
			}
		}
		if (team == 1) {
			for (Player p : world.getPlayers()) {
				p.getActionSender()
						.sendMessage(
								"@red@Blue team is victorious and the war is done! RSCAngel is at peace!");
				handleCapeUnWield(p);
			}
		} else {
			for (Player p : world.getPlayers()) {
				p.getActionSender().sendMessage(
						"@red@The war is over! RSCAngel is at peace!");
			}
		}
	 ...
	 * 
	 * @param team
	 *            determines which team, 0 for red 1 for blue
	 * @param p
	 *            player that joins
	 */
	public void joinTeam(int team, Player p) {
		...
		p.getActionSender()
				.sendMessage(
						"You can get some free food from your base. Talk to the other NPC");
		p.getActionSender().sendMessage(
				"to be sent to the battlefield in wild " + (low ? "15" : "30")
						+ "!");
	}

	/**
	 * Handles all the player logout logic (removes from array, keeps string in
	 * array though)
	 */
	public void handlePlayerLogout(Player p) {
		if (!isRunning())
			return;
		redTeam.remove(p); // Why check when we can simply remove without a
							// problem
		blueTeam.remove(p); // Why check when we can simply remove without a
							// problem
	...
	public void handlePlayerLogin(Player p) {
		...
		if (redTeamPlayernames.contains(p.getUsername())) {
			...
			p.getActionSender()
					.sendMessage(
							"You're at war soldier! Only quitters log out during wars!");
			p.getActionSender().sendMessage(
					"You're no quitter! Get back into the fight!");
	...
	public void handleCapeWield(Player p) {
		...
		if (blueTeam.contains(p)) {
			p.updateWornItems(RED_CAPE.getWieldableDef().getWieldPos(),
					RED_CAPE.getWieldableDef().getSprite());
		} else if (redTeam.contains(p)) {
			p.updateWornItems(BLUE_CAPE.getWieldableDef().getWieldPos(),
					BLUE_CAPE.getWieldableDef().getSprite());
		}
	}
	...
	public boolean inSameTeam(Player p1, Npc n) {
		return ((redTeam.contains(p1) && n.getTeam() == 0) || (blueTeam
				.contains(p1) && n.getTeam() == 1));
	}
	...
	public void spawnProtection(int team, Player p) {
		if (team == 0) {
			...
			final Npc x = new Npc(248, p.getX(), p.getY(), p.getX() - 5,
					p.getX() + 5, p.getY() - 5, p.getY() + 5);
			x.setTeam(team);
			x.setRespawn(false);
	...
	public void leaveTeam(Player player) {
		...
		player.getActionSender().sendMessage(
				"You have left the war, you damn panzy!");
	}
