...
public class CTF {
	...
	/**
	 * Player name array list, used to determine if player was in a team when he
	 * logged out/in
	 */
	...
	public static final int[] FREE_ITEMS = { 221, 222, 223, 224, 367, 188, 11,
			31, 32, 33, 34, 35, 38, 41 };
	public static final int[] DISALLOWED_ITEMS = { 40, 42, 183, 209, 229, 370,
			373, 474, 475, 476, 477, 478, 479, 480, 481, 482, 483, 484, 485,
			486, 487, 488, 489, 490, 491, 492, 493, 494, 495, 496, 497, 498,
			499, 500, 511, 512, 513, 514, 546, 551, 553, 555, 559, 560, 561,
			562, 563, 564, 565, 566, 567, 568, 569, 570, 571, 572, 592, 619,
			637, 638, 639, 640, 641, 642, 643, 644, 645, 646, 647, 648, 649,
			650, 651, 652, 653, 654, 655, 656, 657, 676, 723, 750, 751, 796,
			825, 1213, 1214, 1215, 1216, 1217, 1218, 1288 };
	...
	public void startWarmup(int lvlrank) {
		if (matchRunning || warmupRunning || isEventRunning
				|| World.getWar().isRunning())
			return;
		/**
		 * Initlizie
		 */
		...
		doFlags();
		doChests(true);
		world.sendBroadcastMessage("@yel@Server@whi@",
				"@gre@Capture the Flag@whi@ is about to begin! (Levels "
						+ getLevelString(currentLevelRank) + ")");
		world.sendBroadcastMessage("@yel@Server@whi@",
				"type @yel@::ctf@whi@ to join the event!");
		oldMOTD = Constants.GameServer.MOTD;
		Constants.GameServer.MOTD = "@gre@Capture the Flag is Live.@whi@ (Levels "
				+ getLevelString(currentLevelRank)
				+ ") Type ::ctf to join is elgible.";
	}
	...
	public void handleFlagScore(Player p, GameObject obj) {
		...
		if ((redTeam.contains(p) && obj.getID() == BLUE_FLAG_ID)
				|| (blueTeam.contains(p) && obj.getID() == RED_FLAG_ID)) {
			p.getActionSender()
					.sendMessage(
							"You must steal this flag, then capture it on your own flag");
			return;
		}
		if (((redTeam.contains(p) && obj.getID() == RED_FLAG_ID) || (blueTeam
				.contains(p) & obj.getID() == BLUE_FLAG_ID))
				&& !p.isFlagCarrier()) {
			p.getActionSender().sendMessage(
					"You do not have the enemies flag, get it first!");
			return;
		}
		setGameLocked(true);
		...
		if (blueTeam.contains(p)) {
			...
			sendCTFMessage("@blu@" + p.getUsername()
					+ " @whi@scored 1 point for the @blu@Blue Team");
			p.updateWornItems(
					BLUE_CARRIER_ITEM.getWieldableDef().getWieldPos(),
					p.getPlayerAppearance().getSprite(
							BLUE_CARRIER_ITEM.getWieldableDef().getWieldPos()));
			world.registerGameObject(new GameObject(RED_FLAG, RED_FLAG_ID, 0, 0));
			...
			sendCTFMessage("@red@" + p.getUsername()
					+ " @whi@scored 1 point for the @red@Red Team");
			p.updateWornItems(
					RED_CARRIER_ITEM.getWieldableDef().getWieldPos(),
					p.getPlayerAppearance().getSprite(
							RED_CARRIER_ITEM.getWieldableDef().getWieldPos()));
			world.registerGameObject(new GameObject(BLUE_FLAG, BLUE_FLAG_ID, 0,
					0));
			sendSound("redscores");
			redFlagCarrier = null;
			redScore++;
		...
		if (redTeam.contains(p) && obj.getID() == BLUE_FLAG_ID) {
			if (obj.getLocation().equals(BLUE_FLAG)) {
				...
				if (redFlagCarrier != null) {
					p.getActionSender()
							.sendMessage(
									"There already is a flag carrier, please report this bug");
					return;
				}
				redFlagCarrier = p;
				...
				p.getActionSender().sendMessage(
						"Your defense has been lowered by 75%");
				sendSound(p, "youhavetheflag");
				p.updateWornItems(BLUE_CARRIER_ITEM.getWieldableDef()
						.getWieldPos(), RED_CARRIER_ITEM.getWieldableDef()
						.getSprite());
				for (Player pl : redTeam) {
					sendSound(pl, "yourteamhasflag");
					pl.getActionSender().sendMessage(
							"@red@Your team has the enemy flag!");
				}
				for (Player pl : blueTeam) {
					sendSound(pl, "enemyhasyourflag");
					pl.getActionSender().sendMessage(
							"@red@Red team has your flag!");
				}
			}
			...
			if (obj.getLocation().equals(RED_FLAG)) {
				...
				p.getActionSender().sendMessage(
						"Your defense has been lowered by 75%");
				sendSound(p, "youhavetheflag");
				p.updateWornItems(RED_CARRIER_ITEM.getWieldableDef()
						.getWieldPos(), BLUE_CARRIER_ITEM.getWieldableDef()
						.getSprite());
				for (Player pl : blueTeam) {
					sendSound(pl, "yourteamhasflag");
					pl.getActionSender().sendMessage(
	...
	public void handlePlayerJoin(Player p) {
		if (!isEventRunning) {
			p.getActionSender().sendMessage(
					"Sorry, CaptureTheFlag isn't running at the moment!");
			return;
		}
		if (p.isMod() || p.isPMod()) {
			p.getActionSender().sendMessage(
					"Please use a regular account to join the CTF");
			return;
		}
		...
		if (!isLevelValid(p.getCombatLevel())) {
			p.getActionSender()
					.sendMessage(
							"Sorry, you are not in the correct level rank for CaptureTheFlag");
			p.getActionSender().sendMessage(
					"Currently levels " + getLevelString(currentLevelRank)
							+ " are allowed");
			return;
		}
		boolean cont = true;
		for (InvItem i : p.getInventory().getItems()) {
			for (int in = 0; in < FREE_ITEMS.length; in++) {
				if (i.getID() == FREE_ITEMS[in]) {
					p.getActionSender().sendMessage(
							"You may not bring " + i.getDef().name
									+ " to CaptureTheFlag");
					cont = false;
				}
			}
		...
		if (redTeam.size() > blueTeam.size()) {
			...
			p.getActionSender().sendMessage(
					"@blu@You have joined the blue team!");
				...
				public void action() {
					if (!matchRunning)
						owner.getActionSender().sendMessage(
								"&CaptureTheFlag Will start when Both teams have "
										+ PLAYERS_NEEDED + " players ready");
				}
			});
	...
	public boolean checkAndStart() {
		if (redTeam.size() >= PLAYERS_NEEDED
				&& blueTeam.size() >= PLAYERS_NEEDED) {
	...
	public void handlePlayerLeave(Player p, boolean remove) {
		...
		if (redTeam.contains(p)) {
			p.updateWornItems(
					RED_GEAR_CHEST.getWieldableDef().getWieldPos(),
					p.getPlayerAppearance().getSprite(
							RED_GEAR_CHEST.getWieldableDef().getWieldPos()));
			p.updateWornItems(
					RED_GEAR_LEG.getWieldableDef().getWieldPos(),
					p.getPlayerAppearance().getSprite(
							RED_GEAR_LEG.getWieldableDef().getWieldPos()));
			if (remove)
				redTeam.remove(p);
			...
			p.updateWornItems(
					BLUE_GEAR_CHEST.getWieldableDef().getWieldPos(),
					p.getPlayerAppearance().getSprite(
							BLUE_GEAR_CHEST.getWieldableDef().getWieldPos()));
			p.updateWornItems(
					BLUE_GEAR_LEG.getWieldableDef().getWieldPos(),
					p.getPlayerAppearance().getSprite(
							BLUE_GEAR_LEG.getWieldableDef().getWieldPos()));
			if (remove)
				blueTeam.remove(p);
		...
		p.getActionSender().sendMessage(
				"All your free items have been removed, thanks for playing");

		for (InvItem i : p.getInventory().getItems()) {
			if (i.isWielded())
				p.updateWornItems(i.getWieldableDef().getWieldPos(), i
	...
	public void handlePlayerLogin(Player p) {
		if (!isEventRunning) {
			if (!redTeamPlayernames.contains(p.getUsername())
					&& !blueTeamPlayernames.contains(p.getUsername())) {
		...
		if (!redTeamPlayernames.contains(p.getUsername())
				&& !blueTeamPlayernames.contains(p.getUsername())
				&& isInCaptureTheFlagBoundary(p.getX(), p.getY())) {
			handlePlayerLeave(p, true);
			return;
		}
		...
		if (redTeamPlayernames.contains(p.getUsername())) {
			...
			p.getActionSender()
					.sendMessage(
							"Don't run off during a match! If you want to leave type ::leave");
	...
	public void handlePlayerClothing(Player p) {
		...
		if (blueTeamPlayernames.contains(p.getUsername())) {
			p.updateWornItems(BLUE_GEAR_LEG.getWieldableDef().getWieldPos(),
					BLUE_GEAR_LEG.getWieldableDef().getSprite());
			p.updateWornItems(BLUE_GEAR_CHEST.getWieldableDef().getWieldPos(),
					BLUE_GEAR_CHEST.getWieldableDef().getSprite());
		} else if (redTeamPlayernames.contains(p.getUsername())) {
			p.updateWornItems(RED_GEAR_LEG.getWieldableDef().getWieldPos(),
					RED_GEAR_LEG.getWieldableDef().getSprite());
			p.updateWornItems(RED_GEAR_CHEST.getWieldableDef().getWieldPos(),
					RED_GEAR_CHEST.getWieldableDef().getSprite());
		}
	}
	...
	/**
	 * Determines if the game is locked or not (someone captures the flag and
	 * everyone should be returned)
	 * 
	 * @return
	 */
	...
	public void handleDeath(Player opponent) {
		...
		if (!firstblood) {
			...
			if (red)
				sendCTFMessage("@red@" + opponent.getUsername()
						+ " @whi@from the @red@Red@whi@ team draws FIRST BLOOD");
			if (!red)
				sendCTFMessage("@blu@"
						+ opponent.getUsername()
						+ " @whi@from the @blu@Blue@whi@ team draws FIRST BLOOD");
		}
		if (blueDeaths == DEATHS_FOR_HUMULIATION
				|| blueDeaths == DEATHS_FOR_HUMULIATION * 2) {

			sendSound("humuliation");
			sendCTFMessage("@blu@Blue@whi@ has had " + blueDeaths
		...
		if (redDeaths == DEATHS_FOR_HUMULIATION
				|| redDeaths == DEATHS_FOR_HUMULIATION * 2) {

			sendSound("humuliation");
			sendCTFMessage("@red@Red@whi@ has had " + redDeaths
					+ " deaths without a kill");
	...
	public void handleKill(Player p) {
		...
		if (p.killStreak == RAMPAGE_KILLS) {
			...
			if (redTeam.contains(p))
				sendCTFMessage("@red@" + p.getUsername()
						+ " @whi@from the @red@Red @whi@team has "
						+ RAMPAGE_KILLS + " kills without dying");
			else if (blueTeam.contains(p))
				sendCTFMessage("@blu@" + p.getUsername()
						+ " @whi@from the @blu@Blue @whi@team has "
						+ RAMPAGE_KILLS + " kills without dying");
			...
			if (redTeam.contains(p))
				sendCTFMessage("@red@" + p.getUsername()
						+ " @whi@from the @red@Red @whi@team has "
						+ UNSTOPPABLE_KILLS + " kills without dying");
			else if (blueTeam.contains(p))
				sendCTFMessage("@blu@" + p.getUsername()
						+ " @whi@from the @blu@Blue @whi@team has "
						+ UNSTOPPABLE_KILLS + " kills without dying");
	...
	public boolean inCTF(Player p) {
		return redTeam.contains(p) || blueTeam.contains(p)
				|| isInCaptureTheFlagBoundary(p.getX(), p.getY());
	}
	...
	public boolean isSameTeam(Player p1, Player p2) {
		return (redTeam.contains(p1) && redTeam.contains(p2))
				|| (blueTeam.contains(p1) && blueTeam.contains(p2));
	}

	public static boolean isInCaptureTheFlagBoundary(int x, int y) {
		return (x > MIN_RADIUS.getX() && y > MIN_RADIUS.getY())
				&& (x < MAX_RADIUS.getX() && y < MAX_RADIUS.getY());
	}

	public void handleOpenChest(Player p, GameObject obj) {
		if (redTeam.contains(p)
				&& (obj.getX() == BLUE_CHEST.getX() && obj.getY() == BLUE_CHEST
						.getY())) {
			p.getActionSender().sendMessage(
					"You can not open the other teams chest");
			return;
		} else if (blueTeam.contains(p)
				&& (obj.getX() == RED_CHEST.getX() && obj.getY() == RED_CHEST
						.getY())) {
			p.getActionSender().sendMessage(
					"You can not open the other teams chest");
			return;
		...
		if (p.isFlagCarrier()) {
			p.getActionSender().sendMessage(
					"You have the flag, you are not allowed to use this");
			return;
		}
		p.getActionSender().sendMessage(
				"You open your teams chest and see goods, take your pick");
		String[] options = new String[] { "Strength Potion", "Food",
				"Archery Supplies", "Magic Supplies" };
		p.setMenuHandler(new MenuHandler(options) {
			public void handleReply(final int option, final String reply) {
				if (!CTF.isInCaptureTheFlagBoundary(owner.getX(), owner.getY())) {
					owner.getActionSender().sendMessage("Nice try..");
					Logger.print(owner.getUsername()
							+ " tried to exploit a nasty CaptureTheFlag (fixed) exploit");
					return;
				}
				if (option == 0) {
