...
public final class Player extends Mob {
	...
	public boolean shouldThrowDepositError(int itemid, int amount) {
		if (System.currentTimeMillis() - lastbanktime < 100
	...
	public boolean shouldThrowWithdrawError(int itemid, int amount) {
		if (System.currentTimeMillis() - lastwithdrawtime < 100
	...
	public boolean shouldThrowInvError(int itemposition) {
		if (System.currentTimeMillis() - lastinvtime < 100
				&& lastitemposition == itemposition)
			return false;
				...
				public void run() {
					if (owner.canLogout()
							|| (!(owner.inCombat() && owner.isDueling()) && System
									.currentTimeMillis() - startDestroy > 600000 * 2)) {
						owner.destroy(true);
						matchRunning = false;
					}
				}
		...
		if (mob instanceof Player) {
			...
			if (Instance.getCTF().inCTF(attackerPlayer)
				...
				if (mob instanceof Player) {
					...
					world.registerItem(new Item(20, getX(), getY(), 1, playerr));
	...
	public void teleport(int x, int y, boolean bubble) {
		if (!CTF.isInCaptureTheFlagBoundary(x, y)
				&& CTF.isInCaptureTheFlagBoundary(getX(), getY())) {
			this.getActionSender()
					.sendMessage(
							"You cannot telepot out of the CTF, please use ::leave to leave");
			return;
		}
		if (CTF.isInCaptureTheFlagBoundary(x, y)
				&& !CTF.isInCaptureTheFlagBoundary(getX(), getY())) {
			this.getActionSender().sendMessage(
					"You cannot telepot to CTF area, please use ::ctf");
			return;
		}
		Mob opponent = super.getOpponent();
		if (inCombat()) {
