...
import rscproject.gs.builders.MiscPacketBuilder;
import rscproject.gs.builders.ls.SavePacketBuilder;
...
import rscproject.gs.external.AgilityCourseDef;
...
import rscproject.gs.phandler.client.WieldHandler;
...
import rscproject.gs.util.DuelLog;
...
public final class Player extends Mob {
     ...
     * Sets the mute time.
     *
     * @param muted EPOCH time how long is the player muted. (Multiplied by 1000
     *              in this)
     */
    ...
    */
    ...
    public boolean checkAttack(Mob mob, boolean missile) {
        if (mob instanceof Player) {
            ...
            if (World.getWar().isRunning()
                    && World.getWar().inSameTeam(this, victim)) {
                actionSender
                        .sendMessage("You cannot attack people in the same team as you!");
                return false;
            }
            ...
            if (victim.isNoPK()) {
                actionSender
                        .sendMessage("You cannot attack this RSCA staffer at the moment");
                return false;
            }
    ...
    public void destroy(boolean force) {
        ...
        if (force || canLogout()) {
            ...
            if (getUsername() != null) {
                Instance.getCTF().handleFlagDrop(this);
            }
        } else {
    ...
    public void p2pWildy() {
        if (Constants.GameServer.F2P_WILDY) {
            ...
            for (InvItem i : getInventory().getItems()) {
                if (CTF.isInCaptureTheFlagBoundary(getX(), getY())) {
                    break;
                }
    ...
    public void killedBy(Mob mob, boolean stake) {
        ...
        if (mob instanceof Player) {
            Player attackerPlayer = (Player) mob;
            if (Instance.getCTF().inCTF(attackerPlayer)
                    && this instanceof Player
                    && Instance.getCTF().inCTF((Player) this)) {

                if (mob instanceof Player) {
                    Player playerr = (Player) mob;
                    playerr.getActionSender().sendMessage(
                            "You have defeated " + getUsername() + "!");

                    playerr.getActionSender().sendSound("victory");
                    world
                            .registerItem(new Item(20, getX(), getY(), 1,
                                    playerr));
                    Instance.getDelayedEventHandler().add(
                            new MiniEvent(playerr) {
                ...
                Instance.getCTF().handleKill((Player) super.getOpponent());
                Instance.getCTF().handleDeath(this);
                ...
                if (Instance.getCTF().isRedTeam(this)) {
                    teleportCTF(CTF.RED_SPAWN.getX(), CTF.RED_SPAWN.getY(),
                            false);
                } else if (Instance.getCTF().isBlueTeam(this)) {
                    teleportCTF(CTF.BLUE_SPAWN.getX(), CTF.BLUE_SPAWN.getY(),
                            false);
                }
                resetPath();
        ...
        if (mob instanceof Player) {
            ...
            Instance.getServer().getLoginConnector().getActionSender().logKill(
        ...
        if (stake) {
            ...
            for (InvItem item : duelOffer) {
                ...
                final long playerhash = DataConversions.usernameToHash(player
                        .getUsername());
                DuelLog.sendlog(playerhash, usernameHash, item.getID(), item
                        .getAmount(), getX(), getY(), 2);
            ...
            World.getWar().handleCapeWield(this);
        } else {
            inventory.sort();
    ...
    // p.save
    public void teleportCTF(int x, int y, boolean bubble) {
        Mob opponent = super.getOpponent();
        if (inCombat()) {
    ...
    public void teleport(int x, int y, boolean bubble) {
        if (!CTF.isInCaptureTheFlagBoundary(x, y) && CTF.isInCaptureTheFlagBoundary(getX(), getY())) {
            this.getActionSender().sendMessage("You cannot telepot out of the CTF, please use ::leave to leave");
            return;
        }
        if (CTF.isInCaptureTheFlagBoundary(x, y) && !CTF.isInCaptureTheFlagBoundary(getX(), getY())) {
            this.getActionSender().sendMessage("You cannot telepot to CTF area, please use ::ctf");
            return;
        }
        Mob opponent = super.getOpponent();
        if (inCombat()) {
