...
import rscproject.gs.external.NPCDef;
...
public class Npc extends Mob {
	...
	public Npc(NPCLoc loc) {
		...
		if(World.getWar().isNpcAssociated(this)) {
			curHits = World.getWar().getKingHits();
		}
		else {
			curHits = def.getHits();
		}
			...
			for (int y = 0; y < tiles[x].length; y++) {
				...
				if (t != null) {
					for (Player p : t.getPlayers()) {
						...
						if(World.getWar().inSameTeam(p, this)) {
							System.out.println("In same team! | "  + this.getTeam() + " - " + World.getWar().getTeam(p));
							continue;
						}
						if(World.getWar().inATeam(p) && this.getTeam() != 2 && !(p.isBusy() || p.isNonaggro() || now - p.getCombatTimer() < (p.getCombatState() == CombatState.RUNNING || p.getCombatState() == CombatState.WAITING ? 3000 : 500) || !p.nextTo(this))) {
							return p;
						}
	...
	public void updatePosition() {
		if(World.getWar().getQueen(0) != null && World.getWar().getQueen(1) != null) {
			if(World.getWar().getQueen(0).equals(this) || World.getWar().getQueen(1).equals(this)) {
				return;
			}
		}
		...
		if (now - lastMovement > 2200) {
			...
			if (!isBusy() && finishedPath() && rand == 1 && !this.isRemoved()) {
				...
				if(this.getTeam() != 2) {
					Npc q = World.getWar().getQueen(this.getTeam());
					if(!new Point(newX, newY).inBounds(q.getX()-10, q.getY()-10, q.getX()+10, q.getY()+10)) {
						return;
					}
				}
